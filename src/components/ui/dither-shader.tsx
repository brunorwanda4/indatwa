...
interface DitherShaderProps {
	...
	/** Transition duration for hover reveal in ms */
...
export const DitherShader: React.FC<DitherShaderProps> = ({
	...
	// Derive object-fit CSS value for the original image overlay
	...
	return (
		...
		>
			{/* Dithered canvas layer */}
			<canvas
				ref={canvasRef}
				className="absolute inset-0 h-full w-full"
				style={{
					imageRendering: "pixelated",
					transition: `opacity ${revealDuration}ms ease`,
					opacity: revealOnHover && isHovered ? 0 : 1,
				}}
				aria-hidden={revealOnHover && isHovered}
				aria-label="Dithered image"
				role="img"
			/>

			{/* Original image overlay — shown on hover */}
			{revealOnHover && (
				<img
					src={src}
					alt="Original image"
